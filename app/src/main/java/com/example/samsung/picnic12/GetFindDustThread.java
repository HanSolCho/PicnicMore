...
class GetFindDustThread extends Thread {	//기상청 연결을 위한 스레드
	...
	String[] sDate,sSo2Value,sCoValue,sO3Value,sNo2Value,sPm10Value,sKhaiValue,sKhaiGrade,sSo2Grade,sCoGrade,sO3Grade,sNo2Grade,sPm10Grade,sPm25Value,sPm25Grade;	//예보시간,날짜,온도,풍향,습도,날씨
	boolean bTotalCount,bDate,bSo2Value,bCoValue,bO3Value,bNo2Value,bPm10Value,bKhaiValue,bKhaiGrade,bSo2Grade,bCoGrade,bO3Grade,bNo2Grade,bPm10Grade,bPm25Value,bPm25Grade;	//여긴 저장을 위한 플래그들
	...
	public GetFindDustThread(boolean receiver,String dong){
		...
		bTotalCount=bDate=bSo2Value=bCoValue=bO3Value=bNo2Value=bPm10Value=bKhaiValue=bKhaiGrade=bSo2Grade=bCoGrade=bO3Grade=bNo2Grade=bPm10Grade=bPm25Value=bPm25Grade=false;	//부울상수는 false로 초기화해주자
	}
	public void run(){
		...
		if(active){
			try{
				...
				sPm25Value=new String[100];
				sPm25Grade=new String[100];
				data=0;
				...
				while(eventType!= XmlPullParser.END_DOCUMENT){	//문서의 끝이 아닐때
					...
					case XmlPullParser.START_TAG:	//'<'시작태그를 만났을때
						...
						} if(xpp.getName().equals("pm25Value")){	//미세먼지 지수
							bPm25Value=true;
						...
						}if(xpp.getName().equals("pm25Grade")){	//미세먼지 지수
						//Log.d("초미세먼지","!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!");
							bPm25Grade=true;
		...
		handler.post(new Runnable() {	//기본 핸들러니깐 handler.post하면됨
			...
			public void run() {
				...
				if(tResponse){		//문서를 다 읽었다
					tResponse=false;
					data=0;		//
					Log.d("초미세먼지",sPm25Value[data]);

					Log.d("미세먼지",sPm10Grade[data]);
					Log.d("초미세먼지",sPm25Grade[data]);


					FirstFragment.FindDustThreadResponse(sTotalCount,sDate,sSo2Value,sCoValue,sO3Value,sNo2Value,sPm10Value,sKhaiValue,sKhaiGrade,sSo2Grade,sNo2Grade,sCoGrade,sO3Grade,sPm10Grade,sPm25Value,sPm25Grade);
