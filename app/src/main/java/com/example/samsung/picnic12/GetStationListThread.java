...
class GetStationListThread extends Thread {    //기상청 연결을 위한 스레드
    ...
    public GetStationListThread(boolean receiver, String sido) {
        ...
        stationUrl = getInfo + getStationFindDust + Servicekey + "&" + infoCnt + "&" + addr;
                    ...
                    switch (eventType) {
                        ...
                        case XmlPullParser.TEXT:    //텍스트를 만났을때
                            ...
                            if (bStationName) {                //동네이름
                             //   Log.d("i값 확인", Integer.toString(i));
                                sStationName[data] = xpp.getText();
                             //  data++;
                           //     Log.d("xpp",xpp.getText());
                            //    Log.d("sStationName[date]",sStationName[data]);
        ...
        handler.post(new Runnable() {    //기본 핸들러니깐 handler.post하면됨
            ...
            public void run() {
                ...
                if (tResponse) {        //문서를 다 읽었다
                 ...
                 //   Log.e("stationname", sStationName[data]); // 로그가 안찍힘..
                    FirstFragment.StationListThreadResponse(sTotalCount, sStationName);
